...
const CONFIG = {
    TOTAL_POOL: 10000,
    INITIAL_MINING_POWER: 0.1000,
    UPDATE_INTERVAL: 1000,
    MINERS: [
        { id: 1, name: "Базовый майнер", basePower: 0.1000, basePrice: 10 },
        { id: 2, name: "Продвинутый майнер", basePower: 0.5000, basePrice: 50 },
        { id: 3, name: "Супер майнер", basePower: 2.0000, basePrice: 200 },
        { id: 4, name: "Мега майнер", basePower: 10.0000, basePrice: 1000 }
    ]
};
...
function upgradeMiner(minerId) {
    ...
    const upgradePrice = miner.basePrice * gameState.miners[minerId].level * 2;
    
    if (gameState.userBalance >= upgradePrice) {
        gameState.userBalance -= upgradePrice;
        gameState.miners[minerId].level++;
        
        // Увеличиваем мощность майнера на 50% за каждый уровень
        const powerIncrease = miner.basePower * 0.5;
        gameState.miningPower += powerIncrease;
        ...
        notification.innerHTML = `
            ...
            <div class="achievement-info">
                <h4>Улучшение майнера</h4>
                <p>Мощность увеличена на ${(powerIncrease).toFixed(4)} USDT/с</p>
            </div>
        `;
